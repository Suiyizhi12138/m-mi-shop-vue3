  ...
  <div class="product-page">
    ...
    <header class="header-fixed" v-show="isFixedHeader">
      ...
      <div class="header-nav">
        ...
        <span class="btn-comment" v-show="false" :class="{'btn-active':currentDivIndex==1}"
          @click="jumpDiv(1)">评价</span>
        ...
        <div class="action-box action-type">
          ...
          <div class="action-content">
            <div class="action-content-left" @click="chooseType">
              <span class="product-type">{{product.zh_name}}
                {{product.skus?product.skus[skuIndex].name.replace(/-/,' '):''}}</span>
        ...
        <div class="action-box action-address">
          ...
          <div class="action-content" @click="showAddressPop">
            <div class="action-content-left">
              ...
              <span class="locate-city">{{selectedAddress?selectedAddress.district:"北京市 东城区"}}</span>
    ...
    <!-- 商品评价 -->
    <div class="product-comment" v-show="false">
    ...
    <div class="product-detail-info">
      <div class="detail-info-title .flex-between">
        <h4 class="title-description" @click="showDetailContent('description')"
          :class="{'title-active':isDetailDescription===true}">商品介绍</h4>
        <h4 class="title-params" @click="showDetailContent('params')" :class="{'title-active':isDetailParams===true}">
          商品参数</h4>
      </div>
      <!-- //详情图片 -->
      <transition name="fade-in" mode="out-in">
        <div class="detail-description" v-if="isDetailDescription" key="des">
          <div class="show-fail" v-if="loadProductStatus==2&&product.details.length==0">抱歉,暂无</div>
        ...
        <div class="detail-params" v-else key="par">
          <div class="show-fail" v-if="loadProductStatus==2&&product.parameters.length==0">抱歉，暂无</div>
          <ul class="img-list" v-else>
    ...
    <div class="footer-btns">
      ...
      <div class="footer-right">
        <button class="btn-cart" @click="showChooseType">加入购物车</button>
    ...
    <van-popup v-model:show="isShowTypePop" position="bottom" class="van-popup-type" :style="{height: '80%'}">
      <div class="type-content scroll-hidden">
        ...
        <div class="product-info-box flex-start">
          ...
          <div class="info-right flex-column">
            ...
            <p class="product-name">{{product.zh_name}} {{product.skus[skuIndex].name.replace(/-/,' ')}}</p>
          </div>
        </div>
          ...
          <div class="type-item" v-for="(values,pKey) in product.productinfos.product_info">
            ...
            <ul class="type-value-list">
              <li class="type-value-item" v-for="(item,key) in values.split('-')">
                <a href="javascript:" :class="{'type-value-active': item==selectedType[pKey]}"
                  @click="selectValue(item,pKey)">{{item}}</a>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div class="footer-btn-box">
          <button class="btn-ensure" @click="handleEnsure(clickSource)">确定</button>
        </div>
      </div>
    </van-popup>
    <van-popup v-model:show="isShowAddressPop" position="bottom" closeable class="van-popup-address"
      :style="{height: '80%'}">
      <div class="address-content">
        <h4 class="address-title">收货地址</h4>
        <ul class="address-list">
          <li class="address-item" v-for="(item,index) in addresses" :key="index">
            <a href="javascript:" class="item-link" :class="{'router-link-active': false}" @click="selectAddress(item)">
              <div class="item-left">
...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      //显示选择商品类型
      const showChooseType = () => {
        state.isShowTypePop = true;
        state.clickSource = 'cart'
      };
      //显示选择收货地址
        ...
        switch (source) {
          ...
          case "buy":
            Session.setStore("_buy_item",{
              ...
              amount: state.buyCount,
              product_sku: state.product.skus[state.skuIndex]  
            })
      ...
      const selectAddress = (address) => {
        sessionStorage.setItem('_selected_address', JSON.stringify(address))
        state.isShowAddressPop = false
        state.selectedAddress = address
      }
      const selectedAddress = computed(() => {
        return store.getters.selectedUserAddress
      })
    ...
    .van-popup-type {
      ...
      .type-content {
        ...
        .footer-btn-box {
          ...
          .btn-ensure {
            border-radius: 20px;
            background-image: linear-gradient(90deg, #ff7d00, #ff6700);
            color: #fff;
          }
