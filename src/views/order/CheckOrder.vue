<template>
  <div class="check-order-page">
      ...
      <div class="address-choice-left flex-wrap" @click="showAddressChoice">
        ...
        <div class="user-address">
          <div
            class="address-tips"
            v-if="computeAddress"
          >{{computeAddress?computeAddress.district:'' }} {{ computeAddress?computeAddress.address:''}}</div>
          <div
            class="address-tips"
            v-if="currentAddress"
          >{{currentAddress?currentAddress.district:'' }} {{ currentAddress?currentAddress.address:''}}</div>

          <div class="address-tips" style="color: #ff6700" v-if="!currentAddress && !computeAddress">请选择收货地址</div>
        </div>
      </div>
    ...
    <div class="order-list-container">
      <ul class="order-list" v-if="Array.isArray(buyItems)&&buyItems.length>0">
        <li class="order-list-item flex-between" v-for="item in buyItems" :key="item.id">
          ...
          <div
            class="product-name-box flex-wrap"
          >{{item.product_sku.product.zh_name}} {{item.product_sku.name.replace(/-/,' ')}}</div>
          <div class="product-price-box">{{item.amount}}x{{item.product_sku.price}}</div>
        </li>
      </ul>
      <ul class="order-list" v-else>
        <li class="order-list-item flex-between">
          ...
          <div
            class="product-name-box flex-wrap"
          >{{buyItem?buyItem.name:''}} {{buyItem?buyItem.product_sku.name.replace(/-/,' '):''}}</div>
          <div
            class="product-price-box"
          >{{buyItem?buyItem.amount:''}}x{{buyItem?buyItem.product_sku.price:''}}</div>
        </li>
      </ul>
    </div>
    ...
    <footer class="check-order-footer">
      <div class="footer-address" v-if="computeAddress">配送至 {{computeAddress.district }} {{ computeAddress.address}}</div>
      <div class="footer-address" v-if="currentAddress">配送至 {{currentAddress.district }} {{ currentAddress.address}}</div>
      <div class="footer-btn-box flex-start">
        <div class="footer-total">
          共{{totalAmount?totalAmount:0}}件 合计：
    ...
    <van-popup
      position="left"
      v-model:show="isShowAddressChoice"
      :style="{ width: '100%',height: '100%'}"
    >
      <address-choice
        @closeAddressChoice="handleCloseAddressChoice"
        @selectOrderAddress="getAddressFromChild"
      ></address-choice>
    </van-popup>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const showAddressChoice = () => {
      if(computeAddress._value) return;
    ...
    //提交订单
    const submitOrder = () => {
      //如果缓存地址不存在 本页地址也不存在 则显示弹窗 如果缓存地址存在则提交缓存地址 否则提交当页获取地址
      if (!computeAddress._value && !state.currentAddress) {
        state.isShowAddressChoice = true;
        return
      }else if(computeAddress._value){
        addOrder(computeAddress._value)
      }else{
        addOrder(state.currentAddress)
      }      
    };
    //参数 地址:address 类型:obj  购买项 items
    const addOrder = (address) =>{
      let data = {};
      let arr = [];
      //从购物车进入
      if (Array.isArray(buyItems._value) && buyItems._value.length > 0) {
